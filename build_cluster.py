...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>울산 버스 군집 흐름 (2026-04-07)</title>
...
<div class="panel" id="title"><h1>울산 버스 군집 흐름</h1>
<p><b>버스 상자=가까운 같은 노선 승객을 묶음</b>(크기=묶인 승객 수, <b>줌인하면 개별로 분리</b>) · 색=버스 유형 · 만차 노선은 붉은 테두리 · <b>정류장 색=혼잡도</b>(초록 여유→노랑→주황→<b style="color:#ff5a5a">빨강 만차</b>) · 박스에 마우스를 올리면 노선·묶인 승객·혼잡률, 정류장은 만차 버스 번호 · 왼쪽 사용자유형=카드 승차 흐름</p>
...
function frame(t){const dt=(t-last)/1000;last=t;fit();
 ...
 // 승객 군집(버스) — 같은 노선·가까운 위치를 묶어 박스화 (맨 앞 레이어)
 const G=26; // 화면 격자(px): 줌인=개별로 분리, 줌아웃=뭉쳐 버스 한 대
 const CL=new Map();
 for(const tr of active){if(!pass(tr))continue;people+=tr[6];
  ...
  const X=PX(pos[0]),Y=PY(pos[1]);if(X<-30||X>W+30||Y<-30||Y>H+30)continue;
  const key=tr[2]+'|'+Math.floor(X/G)+'|'+Math.floor(Y/G);
  let o=CL.get(key);if(!o){o={ri:tr[2],n:0,sx:0,sy:0,c:0};CL.set(key,o);}
  o.n+=tr[6];o.sx+=X;o.sy+=Y;o.c++;}
 let cHit=null,chb=1e9;
 for(const o of CL.values()){const X=o.sx/o.c,Y=o.sy/o.c,trNo=D.routeNos[o.ri];
  const s=sqSideN(o.n);
  x.fillStyle=typeCol(trNo);
  if(night){x.shadowColor=x.fillStyle;x.shadowBlur=Math.min(9,s);}
  x.fillRect(X-s/2,Y-s/2,s,s);x.shadowBlur=0;
  if(MANSET[curHour]&&MANSET[curHour].has(trNo)){x.strokeStyle='#ff2020';x.lineWidth=2;x.strokeRect(X-s/2-1,Y-s/2-1,s+2,s+2);}
  else if(!night){x.strokeStyle='rgba(20,28,48,.5)';x.lineWidth=0.6;x.strokeRect(X-s/2,Y-s/2,s,s);}
  if(hov){const dd=(X-mx2)*(X-mx2)+(Y-my2)*(Y-my2),lim=Math.max(9,s);if(dd<chb&&dd<lim*lim){chb=dd;cHit={ri:o.ri,n:o.n,c:o.c};}}}
 // 툴팁: 정류장(우선) > 승객
 ...
 else if(cHit){const ri=cHit.ri,no=D.routeNos[ri]||'노선',rid=RIDS[ri],rc=rid?RC[rid]:null,cap=busCap(no);
  const lo=(rc&&rc.load)?Math.round(rc.load[curHour]):null,pr=lo!=null?Math.round(lo/cap*100):null;
  const man=MANSET[curHour]&&MANSET[curHour].has(no);
  let inner='<b style="color:'+typeCol(no)+'">'+no+'</b> <span style="opacity:.6">'+busType(no)+'</span>';
  inner+='<br>이 덩어리 승객 <b>'+cHit.n.toLocaleString()+'</b>명 (통행 '+cHit.c+'건)';
  inner+= lo!=null? '<br>'+String(curHour).padStart(2,'0')+'시 평균 차내 재차 '+lo+'명 · 혼잡률 <b style="color:'+congCol(pr)+'">'+pr+'%</b>'+(man?' · <b style="color:#ff5a5a">만차</b>':'') : '';
  tip.innerHTML=inner;tip.style.display='block';tip.style.left=(mcx>W-220?mcx-210:mcx+14)+'px';tip.style.top=(mcy+14)+'px';}else tip.style.display='none';
 // HUD
...
open("ulsan_route_cluster.html","w",encoding="utf-8").write(HTML.replace("__DATA__", json.dumps(d, separators=(",",":"))))
print("ulsan_route_cluster.html", round(os.path.getsize("ulsan_route_cluster.html")/1e6,2),"MB")
