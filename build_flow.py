...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>울산 버스 혼잡 흐름 (2026-04-07)</title>
...
<div class="panel" id="title"><h1>울산 버스 혼잡 흐름</h1>
<p><b>정류장 색=혼잡도</b>(초록 여유→노랑→주황→<b style="color:#ff5a5a">빨강 만차</b>) · <b>버스·목록 색=유형</b>(일반·좌석·리무진·지선·마을·순환) · <b>버스 상자 크기=차내 재차 혼잡률</b>(클수록 혼잡) · 시간이 흐르며 만차 정류장이 이동 · 정류장에 마우스를 올리면 현재 만차 버스 번호, 버스 상자에 올리면 차내 재차/혼잡률 표시 · 왼쪽 사용자유형=카드 승차 흐름</p>
...
function frame(t){const dt=(t-last)/1000;last=t;fit();
 ...
 // 승객 사각형(버스) — 맨 앞 레이어 · 크기 = 차내 재차 혼잡률
 for(const tr of active){if(!pass(tr))continue;people+=tr[6];
  ...
  const trNo=D.routeNos[tr[2]],rid=RIDS[tr[2]],rc=rid?RC[rid]:null;
  const lo=(rc&&rc.load)?(rc.load[h0]*(1-frac)+rc.load[h1]*frac):0;
  const ratio=lo/busCap(trNo)*100;
  const s=sqSideC(ratio),X=PX(pos[0]),Y=PY(pos[1]);
  ...
  else if(!night){x.strokeStyle='rgba(20,28,48,.55)';x.lineWidth=0.6;x.strokeRect(X-s/2,Y-s/2,s,s);}
 ...
 if(stopHit){tip.innerHTML='<b>'+stopHit.nm+'</b><br>'+(D.routeNos[selRoute]||'')+'번 · '+stopHit.seq+'/'+stopHit.tot+'번째 정류장';
 ...
 else if(htr){const ri=htr[2],no=D.routeNos[ri]||'노선',rid=RIDS[ri],rc=rid?RC[rid]:null,cap=busCap(no);
  ...
  inner+= lo!=null? '<br>'+String(curHour).padStart(2,'0')+'시 평균 차내 재차 <b>'+lo+'</b>명 / 정원 '+cap+'<br>혼잡률 <b style="color:'+congCol(pr)+'">'+pr+'%</b>'+(man?' · <b style="color:#ff5a5a">만차 운행</b>':'') : '<br>혼잡 데이터 없음';
  tip.innerHTML=inner;tip.style.display='block';tip.style.left=(mcx>W-210?mcx-200:mcx+14)+'px';tip.style.top=(mcy+14)+'px';}else tip.style.display='none';
 // HUD
...
open("ulsan_route_flow.html","w",encoding="utf-8").write(HTML.replace("__DATA__", json.dumps(d, separators=(",",":"))))
print("ulsan_route_flow.html", round(os.path.getsize("ulsan_route_flow.html")/1e6,2),"MB")
