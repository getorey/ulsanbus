...
def _cap(no):
    ...
    if base.isdigit(): return 40 if len(base)==4 else 50
    m=re.findall(r"\d+",base); cls=int(m[0]) if m else 0
    return 20 if 51<=cls<=99 else 50
try:
    shr=defaultdict(lambda:[0]*24); shl=defaultdict(lambda:[0]*24); shn=defaultdict(lambda:[""]*24)
    hrRuns=[0]*24; buses=[]
    for r in csv.DictReader(open("congestion_20260407.csv",encoding="utf-8-sig")):
        ...
        if ratio>shr[sid][h]: shr[sid][h]=round(ratio); shl[sid][h]=cg; shn[sid][h]=no
    mstops=[]
    for sid in shr:
        c=_resolve(sid)
        if not c: continue
        ...
        mstops.append([round(c[0],5),round(c[1],5),(_bs.get(sid) or "").strip(),rr,shl[sid],shn[sid]])
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>울산 버스 만차 시뮬레이션 (2026-04-07)</title>
...
<div class="panel" id="title"><h1>울산 버스 만차 시뮬레이션</h1>
<p>차내 재차 ÷ 정원(좌석40·시내50·마을20) ≥100% = <b style="color:#ff5a5a">만차</b> · 시간대를 움직이면 만차 정류장·버스가 갱신됩니다 · 배경=인구밀도/주야</p>
...
function renderMancha(h){
 ...
 ms.innerHTML=`<h2>🚍 만차 현황 · ${String(h).padStart(2,'0')}시</h2>`
  ...
  +`<div class="sec">혼잡 등급 (정원 대비)</div><div class="lgd">`
  +`<span class="d" style="background:#ff3a3a"></span>만차 ≥100%<br>`
  +`<span class="d" style="background:#ff8c1e"></span>혼잡 80~99%<br>`
  +`<span class="d" style="background:#ffd54f"></span>보통 50~79%<br>`
  +`<span class="d" style="background:#3a9b7a"></span>여유 &lt;50%</div>`
  +`<div class="sec" style="margin-top:8px">정원: 좌석 40 · 시내 50 · 마을 20</div>`;
 ...
 let rows='';for(const b of list){const p=b[5];
  rows+=`<tr><td class="no">${b[1]}</td><td>${b[2]||'-'}</td><td class="r">${b[3]} / ${b[4]}</td>`
...
function frame(t){const dt=(t-last)/1000;last=t;fit();
 ...
 // 만차/혼잡 정류장 마커 (현재 시각)
 let manHit=null,mhb=160;const pulse=0.5+0.5*Math.sin(t/350);
 for(const s of MAN.stops){const r=(s[3]&&s[3][curHour])||0;if(r<50)continue;
  const X=PX(s[0]),Y=PY(s[1]);if(X<-20||X>W+20||Y<-20||Y>H+20)continue;
  const col=rcol(r),sz=r>=100?6.5:r>=80?5:4;
  if(r>=100){x.beginPath();x.arc(X,Y,sz+3+pulse*5,0,7);x.fillStyle='rgba(255,58,58,'+(0.22*(1-pulse*0.4)).toFixed(3)+')';x.fill();}
  x.beginPath();x.arc(X,Y,sz,0,7);x.fillStyle=col;x.fill();
  x.lineWidth=1;x.strokeStyle=night?'#ffffffcc':'#10131c88';x.stroke();
  if(mx2>=0){const dd=(X-mx2)*(X-mx2)+(Y-my2)*(Y-my2);if(dd<mhb){mhb=dd;manHit={nm:s[2],r:r,no:(s[5]&&s[5][curHour])||'',load:(s[4]&&s[4][curHour])||0};}}}
 ...
 else if(manHit){tip.innerHTML='<b>'+manHit.nm+'</b><br>'+String(curHour).padStart(2,'0')+'시 · '+manHit.no+'번 · 재차 '+manHit.load+'명 · <b style="color:#ff6b6b">'+manHit.r+'%</b>';
  tip.style.display='block';tip.style.left=(mcx>W-200?mcx-190:mcx+14)+'px';tip.style.top=(mcy+14)+'px';}
...
open("ulsan_route_sim.html","w",encoding="utf-8").write(HTML.replace("__DATA__", json.dumps(d, separators=(",",":"))))
print("ulsan_route_sim.html", round(os.path.getsize("ulsan_route_sim.html")/1e6,2),"MB")
