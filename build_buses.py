...
_rec=defaultdict(list)   # rid -> (rs, pos, gs, n, ut, dir)
for path,hdr in [("card_records_20260407.csv",False),("card_records_20260407_etc.csv",True)]:
    ...
    for x in rd:
        ...
        elif ro is not None: dirn=0; pos=ro          # 하차 불명 → 승차 정류장 방향
        ...
        _rec[rid].append((rs,pos,gs,n,ut,dirn))
_rid2ri={}
for i,rr in enumerate(d.get("routeRids",[])):
    if rr and rr not in _rid2ri: _rid2ri[rr]=i
busT=[]
for rid,rs in _rec.items():
    ri=_rid2ri.get(rid)
    if ri is None: continue
    durs=sorted(gs-r for r,pos,gs,n,ut,dn in rs if gs>r)
    ...
    for dn in (0,1):
        sub=[t for t in rs if t[5]==dn]
        if not sub: continue
        taus=[(gs-r)/pos for r,pos,gs,n,ut,_ in sub if pos>0 and gs>r]
        tau=min(max(_st.median(taus) if taus else 90,30),180)
        grp=defaultdict(list)
        for r,pos,gs,n,ut,_ in sub:
            grp[round((r-pos*tau)/W)].append((r,gs,n,ut))
        for b,riders in grp.items():
            dep=b*W; end=dep+travel
            rr2=[(r,min(gs,end),n,ut) for r,gs,n,ut in riders if dep-120<=r<=end+120]
            if not rr2: continue
            t0=round(dep/60-240,2); t1=round(end/60-240,2)
            if t1<=t0: t1=t0+1
            busT.append([t0,t1,ri,dep,travel,ei,dn,[[r,gs,n,ut] for r,gs,n,ut in rr2]])
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>울산 버스 운행 복원 (2026-04-07)</title>
...
<div class="panel" id="title"><h1>울산 버스 운행 복원</h1>
...
function frame(t){const dt=(t-last)/1000;last=t;fit();
 ...
 for(const b of active){const f=(now-b[0])/(b[1]-b[0]);if(f<0||f>1)continue;
  const on=onb(b,tsec);if(on<=0)continue;people+=on;
...
open("ulsan_route_buses.html","w",encoding="utf-8").write(HTML.replace("__DATA__", json.dumps(d, separators=(",",":"))))
print("ulsan_route_buses.html", round(os.path.getsize("ulsan_route_buses.html")/1e6,2),"MB")
